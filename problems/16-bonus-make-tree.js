...
const makeTree = (categories, targetParent) => {
  ...
  for (let i = 0; i < categories.length; i++) {
    ...
    let catId = cat.id
    let catParent = cat.parent

    //if I find one whose parent matches target --> key in obj to the id, value another tree
    if (catParent === targetParent) {
      result[catId] = makeTree(categories, catId)
    }
